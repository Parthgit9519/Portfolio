      ...
      <div className="container mx-auto px-4 flex items-center justify-between h-16">
        ...
        <a href="#home" className="flex items-center">
          <span className="text-3xl font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-purple-700 to-pink-600 transition-all duration-300 hover:opacity-80">
            <img width={60} src="https://media.istockphoto.com/id/1465504312/vector/young-smiling-man-avatar-man-with-brown-beard-mustache-and-hair-wearing-yellow-sweater-or.jpg?s=612x612&w=0&k=20&c=9AyNmOwjadmLC1PKpANKEXj56e1KxHj9h9hGknd-Rb0=" alt="" />
          </span>
        </a>
          ...
          {navLinks.map((link, index) => (
            <li key={index}>
              <NavLink
                to={link.path}
                className={({ isActive }) =>
                  `hover:text-blue-500 font-medium transition ${
            ...
            {navLinks.map((link, index) => (
              <li key={index}>
                <NavLink
                  to={link.path}
                  className={({ isActive }) =>
                    `block font-medium hover:text-blue-500 transition ${
                      isActive ? "text-blue-600" : "text-gray-700"
                    }`
                  }
